...
**MVP(既定)** と **高信頼** の2段階。Goal Frame作成時にFableが提案し、人間がGoal Frame確認時に確定する。マイルストーン単位の一時変更は人間が指示できる。FableゲートとHuman Acceptanceは両強度で維持する(要件適合性の保証線)。一時変更は state.md の強度欄に記録し、そのマイルストーン完了時に goal-frame.md の確定値へ戻す。

| 工程 | MVP(既定) | 高信頼 |
|---|---|---|
| Goal Frame / Goal Gate(A-1 / A-6) | 実施(未知マップ+強度+終了条件を含む) | 同左 |
| マイルストーン開始確認(B-1・Fable) | 実施(軽量) | 同左 |
| 実装委譲(B-2) | E2E価値単位でまとめて codex exec に委譲可(タスク細分化しない) | タスク分解して個別に委譲 |
| タスク単位の受け入れ(B-3) | codex自己検証報告の確認のみ(diff精読なし) | Opusメインがdiffを確認 |
| テスト要求(B-2) | 受け入れ基準に直結する検証+未知低減に効く検証のみ | 単体・結合・lint・型検査をフル要求 |
| 独立レビュー(B-5) | 省略(Opusメインがsubmission作成時にセルフチェック) | Opusサブで実施(PL-003) |
| Implementation Gate(B-6・Fable) | 実施(適合性+残存未知の許容性) | 同左 |
| Human Report / Acceptance(B-7 / B-8) | 実施(判断の内訳つき) | 同左 |

...
| ブレスト〜Plan(A-2〜A-4) | superpowers:brainstormingを**Fable代理回答(ユーザー目線)+技術PM回答(HOW・Codex astra / max)**で実施(人間はASK_HUMAN時のみ。最終の設計承認は代理Fable) | 人間参加のsuperpowers:brainstorming(技術PMは人間が求めた場合のみ) |
...
| 技術PM | Codex `gpt-6-astra` / `max` / read-only | 実装責任者として、HOWに係るQAへ回答する(実装方式・技術選択・構成と分割・技術リスク・実現性・検証可能性) | MVPの代理ブレスト(A-2〜A-4)で、OpusがHOWの問いを束ねて呼び出す(1ラウンド=1回)。高信頼では人間が求めた場合のみ | コード変更・コミット。ユーザー価値・好み・優先順位の決定(`NEEDS_USER_VIEW:` で代理Fableへ返す)。設計承認 |
| Codex | Codex `gpt-6-astra` / B-1でFableが選択(`codex-run.ps1` の既定は `low`) | 実装担当(`codex exec`)。コード変更、検証、自己検証報告 | マイルストーン単位(MVP)またはタスク単位(高信頼)で呼び出し | コミット、要件の再定義、否定リスト該当の自律判断 |
...
3. 代理ブレスト(MVP・A-2〜A-4): superpowers:brainstormingの**ユーザー目線の問い**(WHAT・価値・優先順位)と設計承認に、人間の代理として回答するとき(代理Fable)。HOWの問いは技術PMへ回す
...
| ID | ポリシー | 要件 |
|---|---|---|
| PL-001 | Default roles | Fable=責任/判定/ユーザー目線の代理回答、Opus 5.5=整理/仕様/レビュー/報告、Codex=技術PM(HOW回答)/実装をデフォルトとする |

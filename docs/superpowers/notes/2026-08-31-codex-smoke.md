...
### 新しい観察: judge モデルはリポジトリを探索しようとする

team-lead の再実行時のログから、判定を下す前にモデルが `Get-Content` でファイルを読み、`Get-ChildItem` でディレクトリを列挙し、`rg` でリポジトリ全体を検索するシェルコマンドを発行しようとしたことが確認された(サンドボックスに阻まれて実行自体は失敗したが、意図としては探索していた)。これは、judge を一時ディレクトリに隔離する設計判断(C9)が必須であることの実測的な裏づけである。S5(隔離テスト、OK)と合わせて、**物理隔離(`-C <tmpdir>`)とプロンプトでの探索禁止指示の両方を併用する**方針を採る。
...
- Task 3(policy.md)/ Task 4(SKILL.md): judge は判定前にリポジトリを探索しようとする挙動が実測された(team-leadの再実行ログで確認)。C9(judgeを一時ディレクトリに隔離)を必須とし、あわせてプロンプトで探索禁止を明記する(物理隔離のみに頼らない)。
